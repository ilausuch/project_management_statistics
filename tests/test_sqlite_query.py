...
def test_status_snapshot():
    ...
    issue5 = Issue(issue_id=5, project=1, status="Workable", created_on=date_move_on_in_progress, closed_on=None)
    ...
    # Issue events for issue 5 (not related with status)
    session.add_all([
        IssueEvent(issue_id=5, type="attr", field="other", created_on=date_move_on_resolved,
                   old_value="test", new_value="terst")
    ])
    ...
    assert result[4]["status"] == "Workable"
